...
var _ = Describe("Commander", func() {
	...
	Describe("FlushAll", func() {
		It("should return the real results of a valid FLUSHALL", func() {
			...
			commander := New(conn)
			var flushResult string
			errCmd := commander.FlushAll(&flushResult, nil).Commit()
			conn = getConn()
			var getResult string
			errSendGet := conn.Send("GET", key)
			results, errResultGet := redis.Values(conn.Do(""))
	...
	Describe("XReadGroup", func() {
		...
		It("should return the real results of a valid XReadGroup with NOACK", func() {
			...
			conn = getConn()
			commander = New(conn)
			errCmd2 := commander.XReadGroup(&xreadGroupResult2, groupName, consumerName, []string{"testStream"}, []string{"0-0"}, nil).Commit()
	...
	Describe("XPENDING", func() {
		It("should return the real results of a valid XPENDING", func() {
			...
			conn = getConn()
			commander := New(conn)
			errCmd := commander.XPending(&xpendingResult, "testStream", groupName, nil).Commit()
	...
	Describe("XCLAIM", func() {
		It("should return the real results of a valid XCLAIM", func() {
			...
			conn = getConn()
			commander := New(conn)
			errCmd := commander.XClaim(&xclaimResult, "testStream", groupName, consumerName2, 100*time.Millisecond, []string{xaddResult}, nil).Commit()
	...
	Describe("Integration test command and commit", func() {
		...
		It("should return the results of a valid chain of del and flush commands", func() {
			...
			errCmd := commander.
				Select(&selectResult, 0).
				Set(&setResult1, key1, 9, nil).
				Set(&setResult2, key2, 9, nil).
				Keys(&keysResult, "*Key*").
				Del(&delResult, key1, "NotExistKey").
				Get(&getResult1, key1).
				FlushAll(&flushResult, &FlushAllOption{Async: true}).
				Get(&getResult2, key2).
				Commit()
